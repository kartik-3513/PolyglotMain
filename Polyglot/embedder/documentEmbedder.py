    ...
    def embed(self, path, model):
        ...
        for lang in langs:
            categories = [
                ...
                if os.path.isdir(os.path.join(path + lang, f))
            ]
            for category in categories:
                for filename in os.listdir(path + lang + category):
                    if filename[:-4] != "metadata":
                        with open(
                            os.path.join(path + lang + category, filename),
                            "r",
                            encoding="utf8",
                        ) as file:
                            articles.append(
        ...
        scores["KMeans"] = [
            adjusted_mutual_info_score(list(labels), list(real)),
